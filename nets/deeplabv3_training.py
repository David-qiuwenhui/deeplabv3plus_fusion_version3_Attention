...
def Focal_Loss(inputs, target, cls_weights, num_classes=21, alpha=0.5, gamma=2):
    n, c, h, w = inputs.size()  # (B,C,512,512)
    nt, ht, wt = target.size()  # (B,C,512,512)
    if h != ht and w != wt:
        inputs = F.interpolate(
            inputs, size=(ht, wt), mode="bilinear", align_corners=True
        )
    ...
    temp_inputs = (
        inputs.transpose(1, 2).transpose(2, 3).contiguous().view(-1, c)
    )  # (B,C,512,512) -> (B,512,C,512) -> (2097152, C)
    temp_target = target.view(-1)  # (2097152,)
